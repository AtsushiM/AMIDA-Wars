		...
		//public
		   ...
		   */
			...
			Unit: function(config){
				...
				sprite.checkMoveSquere = function(){
					var map_size = CONST_CASH.MAP.CHIP_SIZE,
						unit_size_diff_x = CONST_CASH.UNIT.CHIP_SIZE / 2,
						unit_size_diff_y = unit_size_diff_x / 4,
						ret = false;

					if(!(Math.floor(this.x - unit_size_diff_x) % map_size) && !(Math.floor(this.y - unit_size_diff_y) % map_size)){
						var now = this.mapPoint(),
							before = this.beforePoint;
						if(now.x !== before.x && now.y !== before.y){
							this.beforePoint = now;
							ret = true;
						}
					}
					return ret;
				...
				sprite.walk_count = 0;
				sprite.walk_true = 0;
				sprite.default_frame = sprite.frame;
				sprite = AW.propOverride(sprite,config);

				//set before map squere point
				sprite.beforePoint = sprite.mapPoint();

				sprite.walk = function(){
					if(!(GAME.frame % 5)){
						this.walk_count++;

						if(this.walk_count === 4){
							this.walk_count = 0;
						}
						this.walk_true = this.walk_count;
						if(this.walk_count === 3){
							this.walk_true = 1;
						}
					}
					
					this.frame = this.default_frame + this.walk_true * line_num + this.direction;
				};
				...
				if(mode === have.USER){
					...
					sprite.move = function(){
						...
						if(this.direction === 0){
							this.y -= this.speed;
							if(this.checkMoveSquere()){
								/* chip_direction = sprite. */
							}
						}
